...
val title : ?fname:string -> fields -> (string, [> Rresult.R.msg ]) result
...
val date : ?fname:string -> fields -> (Ptime.t, [> Rresult.R.msg ]) result
...
(** [parse_filename f] parses a Jekyll format filename
    ...
    components respectively. If the time could not be parsed, then an
    error is returned. *)
